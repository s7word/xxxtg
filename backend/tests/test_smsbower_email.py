...
class TestAttestationEmailFallback(unittest.IsolatedAsyncioTestCase):
    def _config(self, **overrides):
        base = dict(
            ...
            email_provider_mode="reghelp_primary",  # explicit: this case tests REGHelp → SMS Bower fallback
    ...
    async def test_smsbower_primary_used_first(self):
        cfg = self._config(email_provider_mode="smsbower_primary")
        gateway = AttestationGatewayService(cfg)
        try:
            gateway.smsbower.get_login_email = AsyncMock(
                return_value=EmailInboxResult(email="first@gmail.com", task_id="7")
            )
            gateway.reghelp.get_login_email = AsyncMock(
                side_effect=AssertionError("reghelp should not be called when SMS Bower succeeds")
            )
            inbox = await gateway.get_login_email({}, "+1")
            self.assertEqual(inbox.email, "first@gmail.com")
            self.assertEqual(gateway.last_used_email_provider, "smsbower")
            gateway.smsbower.get_login_email.assert_awaited_once()
            gateway.reghelp.get_login_email.assert_not_called()
        finally:
            await gateway.close()
