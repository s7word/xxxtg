"""SMS Bower (smsbower.app) 接码 + 临时 Google 邮箱客户端。
...
临时邮箱（SetUpEmailRequired，Google / gmail.com）：
  ...
  GET https://smsbower.page/api/mail/getPriceRests
"""
...
class SmsBowerService(GrizzlySmsService):
    ...
    async def get_login_email(
        ...
        if log_callback:
            ...
            await log_callback(f"SMS Bower Google 邮箱已分配 (mailId={task_id}){price_hint}: {mail}")
